...
export default function FacebookCompetitorAnalysis({ backendUrl = "" }) {
  ...
  const normalizeQuery = (raw) => {
    ...
    try {
      ...
      if (parts.length && !["share", "groups", "watch", "profile.php", "pages"].includes(parts[0])) {
        return parts[0];
      }
      if (parts[0] === "profile.php") {
        return u.searchParams.get("id") || value;
      }
      return value;
    } catch {
      return value;
  ...
  const analyze = async () => {
    ...
    try {
      const res = await fetch(
        `${backendUrl}/api/v1/facebook/competitor/${encodeURIComponent(clean)}`
      );
      const json = await res.json();
      if (!res.ok) {
        throw new Error(json.detail || "Failed to fetch Facebook competitor data.");
      }
      setData(json);
    } catch (err) {
  ...
  return (
    <section className="mt-10 pt-8 border-t border-gray-200">
      <div className="mb-6">
        <h2 className="text-xl font-bold text-gray-900">Facebook Competitor Analysis</h2>
        <p className="text-sm text-gray-500 mt-1">
          Analyze a public Facebook page by username, page name, page ID, or Facebook URL.
        </p>
      </div>
      ...
      <div className="flex gap-3 mb-6">
        <input
          ...
          placeholder="nike or https://www.facebook.com/share/..."
          className="flex-1 border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-600 focus:border-blue-600 outline-none"
          disabled={loading}
        />
        <button
          onClick={analyze}
        ...
        <div className="space-y-6">
          <div className="bg-white border border-gray-200 rounded-xl p-6">
            <div className="flex flex-col sm:flex-row sm:items-center gap-5">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="text-lg font-bold text-gray-900">{data.name || data.pageId}</h3>
                  {data.isVerified && (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
              ...
              <div className="rounded-lg bg-gray-50 border border-gray-200 p-3 text-center">
                <p className="text-xl font-bold text-gray-900">{fmt(data.fanCount)}</p>
              ...
              <div className="rounded-lg bg-gray-50 border border-gray-200 p-3 text-center">
                <p className="text-xl font-bold text-gray-900">{data.pageId || "-"}</p>
