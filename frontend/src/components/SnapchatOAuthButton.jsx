...
/**
 * SnapchatOAuthButton – Initiates the Snapchat OAuth2 flow.
 *
 * Similar to the existing "+ Add FB/IG" button but points to
 * the Snapchat authorization endpoint on our backend.
 */
export default function SnapchatOAuthButton({ backendUrl = "" }) {
  const [loading, setLoading] = useState(false);

  function handleConnect() {
    setLoading(true);
    // Navigate to the backend's Snapchat OAuth login endpoint
    window.location.href = `${backendUrl}/snap/auth/login`;
  }
  ...
  return (
    <button
      ...
      disabled={loading}
      className="inline-flex items-center gap-2 px-4 py-2 bg-yellow-400 text-gray-900 text-sm font-semibold rounded-lg hover:bg-yellow-500 disabled:opacity-50 transition shadow"
    >
      {/* Snapchat ghost icon */}
      <svg
        className="w-4 h-4"
      ...
      {loading ? "Connecting..." : "+ Add Snapchat"}
    </button>
  );
}
